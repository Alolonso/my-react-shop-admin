...
const Pagination = ({ totalItems, itemsPerPage, neighbours, setOffset }) => {
  ...
  const neighborsOnEachSide = neighbours * 2;
  const neighborsRange = neighborsOnEachSide + 1;
  const end = Math.min(Math.max(neighborsOnEachSide + 2, neighbours + current + 1), totalPage + 1);
  const start = Math.min(Math.max(end - neighborsRange, 1), Math.max(current - neighbours, 1));
        ...
        <div>
          <p className="text-sm text-gray-700">
            <span className="font-medium">
              {`Showing ${itemsPerPage * (current - 1) + 1} 
                to ${current * itemsPerPage < totalItems ? current * itemsPerPage : totalItems} 
